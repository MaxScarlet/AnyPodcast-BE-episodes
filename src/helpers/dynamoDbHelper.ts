...
export class DynamoDbHelper<T> implements IDbHelper<T>{
    ...
    public async find<T>(args: any): Promise<T[]> {
        const params = this.convertAttrsForScan(args);
        return await this.scan<T>(params);
    }

    private async scan<T>(params: DynamoDB.DocumentClient.ScanInput): Promise<T[]> {
        params.TableName = this.tableName;
        const result = await this.db.scan(params).promise();
        return <T[]>result.Items;
    }

    private async put(item: DynamoDB.DocumentClient.PutItemInputAttributeMap): Promise<void> {
        const params = {
            TableName: this.tableName,
            Item: item,
        };
    ...
    private convertAttrsForScan(obj: any): DynamoDB.DocumentClient.ScanInput {
        const params = { TableName: "", FilterExpression: "", ExpressionAttributeNames: {}, ExpressionAttributeValues: {} };
        let i = 1;
        for (const key in obj) {
            if (obj.hasOwnProperty(key)) {
                const value = obj[key];
                params.FilterExpression += (params.FilterExpression ? " AND " : "") + `#field${i}=:value${i}`;
